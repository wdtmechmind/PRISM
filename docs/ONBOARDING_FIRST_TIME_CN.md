...
本指南面向从未使用过 PRISM 的同学。

- 目标：从 0 到可跑通“采集 -> 重建 -> 分析 -> 仿真 replay -> 真机 replay”。
- 假设：仓库已在本机，路径为 `/mnt/projects-8tb/PRISM`。
- 约束：硬件接线与设备上电步骤由你自行维护，本指南只留占位。

---

## 0. 你填写：Hardware Setup（占位）

请在这里补充你自己的硬件准备 SOP（本文件故意不替你写）：

1. 相机连接与供电检查。
2. 触发线连接检查（主从相机）。
3. UR3 与末端工具上电/网络检查。
4. RPi 网络与串口/UDP 通道检查。
5. 安全区域和急停检查。

建议你把该 SOP 放在 `docs/HARDWARE_SETUP_LOCAL.md`，并在团队内统一版本。
...
推荐环境是 `camera`，也可以用你自己的环境。

```bash
# 可选：如果你使用 conda/mamba
